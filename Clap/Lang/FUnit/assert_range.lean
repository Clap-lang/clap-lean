...
/-- Range-check `e` to `w` bits. Old model: `F.assert_range` (`Clap/Lang.lean:25`), where
...
`num2bits` gate asserts nothing: `stepNum2bits` (`Clap/eDSLState/CircuitEvalSt.lean:412`)
...
(`Clap/eDSLState/ConstraintSystem/num2bits.lean`) emits `bits2num(bits) - expr` alongside the
...
`Primes.bn254` are `sorry`'d in `Clap/Primes.lean`, and `native_decide` refuses anything
depending on `sorry`. -/
