...
def a        : Circuit 7 var := .lam fun x => .eq0 (.v x + .c 1) (.eq0 (.v x + .c 2) (.eq0 (.v x + .c 1) .nil ))
def expected : Circuit 7 var := .lam fun x => .eq0 (.v x + .c 1) (.eq0 (.v x + .c 2) .nil )

#guard s!"{@dedup 7 Nat a}" = s!"{@expected Nat}"
