...
instance {p} : Inhabited (Edsl.Lang.F p) := ⟨Exp.c 0⟩
...
def ark {t c : ℕ} (state : Vector (Edsl.Lang.F p) t) (C : Vector (Edsl.Lang.F p) c) (r : ℕ) : Vector (Edsl.Lang.F p) t :=
...
def mix {t : ℕ} (state : Vector (Edsl.Lang.F p) t) (M : Vector (Vector (Edsl.Lang.F p) t) t) : Vector (Edsl.Lang.F p) t :=
  state.mapIdx (fun (i : ℕ) _ ↦
    (state.zipWith (fun (sj : Edsl.Lang.F p) (row : Vector (Edsl.Lang.F p) t) ↦ row[i]! * sj) M).sum)

-- TODO: Imperative or functional style?
-- def mix (state : Array (Edsl.Lang.F p)) (m : Array (Array (Edsl.Lang.F p))) : Array (Edsl.Lang.F p) := Id.run do
--   let t : ℕ := state.size               -- TODO this will be known at compile time?
--   let mut out : Array (Edsl.Lang.F p) := #[]
--   for i in [:t] do
--     let mut acc : Edsl.Lang.F p := 0
--     for j in [:t] do
...
def mixLast {t : ℕ} (state : Vector (Edsl.Lang.F p) t) (M : Vector (Vector (Edsl.Lang.F p) t) t) (s : ℕ) : Edsl.Lang.F p :=
  (state.zipWith (fun (sj : Edsl.Lang.F p) (row : Vector (Edsl.Lang.F p) t) ↦ row[s]! * sj) M).sum

-- TODO: Imperative or functional style?
-- def mixLast (s : ℕ) (state : Array (Edsl.Lang.F p)) (m : Array (Array (Edsl.Lang.F p))) : Edsl.Lang.F p := Id.run do
--   let mut acc : Edsl.Lang.F p := 0
...
def mixS {t s : ℕ} (r : ℕ) (state : Vector (Edsl.Lang.F p) t) (S : Vector (Edsl.Lang.F p) s) : Vector (Edsl.Lang.F p) t :=
  -- let t : ℕ := state.length
  let base : ℕ := (2 * t - 1) * r
  ⟨#[dotProduct base] ++ (tail base).toArray, sorry⟩
where
  /-- `out[0] = Σᵢ S[base + i] · in[i]` — full dot product for element 0 -/
  dotProduct (base : ℕ) : Edsl.Lang.F p :=
    let s' : Vector _ t := ⟨S.extract base (base+t) |>.toArray, sorry⟩
    (state.zipWith (· * ·) s').sum
  /-- `out[i] = in[i] + in[0] · S[base + t + i − 1]` for `i ∈ [1, t)` -/
  tail (base : ℕ) : Vector (Edsl.Lang.F p) (t-1) :=
    (state.drop 1).mapIdx (fun i sᵢ ↦ sᵢ + state[0]! * S[base + t + i]!)

-- TODO: Imperative or functional style?
-- def mixS (r : ℕ) (state : Array (Edsl.Lang.F p)) (s : Array (Edsl.Lang.F p)) : Array (Edsl.Lang.F p) := Id.run do
--   let t := state.size
--   let base := (2 * t - 1) * r
--   let mut out : Array (Edsl.Lang.F p) := #[dotProduct base t]
--   for i in [1:t] do
--     out := out.push (sparseCorrection base t i)
--   return out
-- where
--   /-- `out[0] = Σᵢ S[base + i] · in[i]` — full dot product for element 0 -/
--   dotProduct (base t : ℕ) : Edsl.Lang.F p := Id.run do
--     let mut acc : Edsl.Lang.F p := 0
--     for i in [:t] do
--       acc := acc + s[base + i]! * state[i]!
--     return acc
--   /-- `out[i] = in[i] + in[0] · S[base + t + i − 1]` — identity + in[0] correction -/
--   sparseCorrection (base t i : ℕ) : Edsl.Lang.F p :=
...
def poseidonEx {n c s : ℕ} (inputs : Vector (Edsl.Lang.F p) n) (initState : Edsl.Lang.F p)
  ...
  let t : ℕ := 1 + n
  let nRoundsF : ℕ := 8
  let nRoundsP : ℕ := 4 -- N_ROUNDS_P[t - 2]!
  ...
  let state ← (List.range (half - 1)).foldlM (fun state r ↦ do
    ...
    return mix (ark l C ((r + 1) * t)) M) state
