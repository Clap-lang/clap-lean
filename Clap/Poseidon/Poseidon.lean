...
def mix (state : Array (F p)) (M : Array (Array (F p))) : Array (F p) :=
  state.mapIdx (fun (i : ℕ) _ ↦
    (state.zipWith (fun (sj : F p) (row : Array (F p)) ↦ row[i]! * sj) M).foldl (· + ·) 0)
...
def mixLast (state : Array (F p)) (M : Array (Array (F p))) (s : ℕ) : F p :=
  (state.zipWith (fun (sj : F p) (row : Array (F p)) ↦ row[s]! * sj) M).foldl (· + ·) 0
...
where
  ...
  dotProduct (base : ℕ) : F p :=
    (state.zipWith (· * ·) (s.extract base (base + state.size))).foldl (· + ·) 0
  /-- `out[i] = in[i] + in[0] · S[base + t + i − 1]` for `i ∈ [1, t)` -/
  tail (base t : ℕ) : Array (F p) :=
    (state.extract 1 t).mapIdx (fun i sᵢ ↦ sᵢ + state[0]! * s[base + t + i]!)
...
/-- **PoseidonEx:** Full Poseidon permutation
    ...
    - `C`         — flat array of all round constants
    - `S`         — flat array of sparse-matrix entries for partial rounds
...
def poseidonEx (nOuts : ℕ) (inputs : Array (F p)) (initState : F p)
  ...
  (List.range nOuts).toArray.map (mixLast state M)
