...
def schedule (block : Block t.U32) : m (RoundConstantsTable t.U32) :=
  aux block 16
where
  aux (acc : Array t.U32) (i:Nat) : m (Array t.U32) := do
    if i >= K_SIZE then return acc else
    let t16 := acc[i - 16]!
    let t15 := acc[i - 15]!
    let t7 := acc[i - 7]!
    let t2 := acc[i - 2]!
...
where
  aux hash (i:Nat) := do
    ...
    let hash ← shuffle_i m ws hash i
    aux hash (i+1)

def compress (block : Block t.U32) (hash : Hash t.U32) : m (Hash t.U32) := do
  let ws ← schedule m block
  let hash' ← shuffle m ws hash
...
where
  process_blocks (blocks : Array (Block t.U32)) (acc : Hash t.U32) i := do
    ...
    let acc ← compress m blocks[i]! acc
