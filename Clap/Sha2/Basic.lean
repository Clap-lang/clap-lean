...
/-
  ...
  - fixed-length input
  - US type (for USize) is used for generic numerical types that are
    mapped to F
-/
...
def of_nat_be (x:Nat) (len:Nat) : Array (t.U8) :=
  ...
  where
    aux (x:Nat) (len:Nat) : List t.U8 :=
      let d : Nat := x / (2^8)
      let r : Nat := x % (2^8)
      let r : t.US := OfNat.ofNat r -- does not wrap as r < 256
      if len=0 then [] else
      r::(aux d (len-1))

def sigma_constants : Array USize := #[7, 18, 3, 17, 19, 10]

def sigma (c0 c1 c2 : USize) (x : t.U32) : t.U32 :=
...
def sum_constants : Array USize := #[2, 13, 22, 6, 11, 25]

def sum (c0 c1 c2 : USize) (x : t.U32) : t.U32 :=
  Sha.xor3 (Sha.rotR c0 x)
           (Sha.rotR c1 x)
           (Sha.rotR c2 x)
...
def round_constants_224_256 : RoundConstantsTable t.US :=
...
def padding (msg : Array t.U8) : Array t.U8 :=
  ...
  let k_zero_bytes : Array t.U8 :=
    ...
    Array.replicate n_zero_bytes (0:t.US)
  let k_one_byte : t.U8 := (128:t.US) -- one byte with a single 1 as msb
