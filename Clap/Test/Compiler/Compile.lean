...
def Compile.ex₀ {p : Nat} [Fact (Nat.Prime p)] (point₁ point₂ : Point p) (point₃ : Point' p) : Option Unit := do
  eq0 (point₁.x + point₃.w)
  eq0 (point₂.x + point₁.z)
  accept
...
/--
info:
def Compile.ex₀_ser : (p : ℕ) → [Fact (Nat.Prime p)] → (var : Type) → Circuit p var :=
...
/--
info:
def Compile.ex₀_ser_wg : {p : ℕ} →
  [Fact (Nat.Prime p)] → Compile.Point p → Compile.Point p → Compile.Point' p → Array (ZMod p) :=
  fun {p : ℕ} [Fact (Nat.Prime p)] (point₁ point₂ : Compile.Point p) (point₃ : Compile.Point' p) =>
