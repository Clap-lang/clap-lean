...
def Curry.ex₁ {p : Nat} (p₁ : Vector (ZMod p) 2) (p₂ : Vector (ZMod p) 3) : Option Unit := do
  Spec.Compiler.eq0 (p₁[0] + p₂[2])
  Spec.Compiler.accept

/--
info: def -
  fun {p : ℕ} (curried0_p₁ curried1_p₁ curried0_p₂ curried1_p₂ curried2_p₂ : ZMod p) => do
    Spec.Compiler.eq0 (curried0_p₁ + curried2_p₂)
    some Spec.Compiler.accept
---
info: type -
  {p : ℕ} → ZMod p → ZMod p → ZMod p → ZMod p → ZMod p → Option Unit
-/
