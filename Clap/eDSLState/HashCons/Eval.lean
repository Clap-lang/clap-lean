...
:=
  ...
  | .none =>
    ...
    | .some expr =>
      ...
      -- for branch nodes, recurse, adding the children's values if needed
...
lemma run_eval_idxOf_c_of_contains {ref : HashConsM p ExprRef} {e : CacheExpr p}
  (h : (ref.run σ).2.exprs.contains e)
  (h': (ref.run σ).1 = (ref.run σ).2.exprs.idxOf e)
:
  ref.runGet? σ = .some e
:= by
...
@[grind .]
lemma run_eval_idxOf_c_of_contains' {ref : HashConsM p ExprRef} {e : CacheExpr p}
  (h : (ref σ).2.exprs.contains e)
  (h': (ref σ).1 = (ref σ).2.exprs.idxOf e)
:
  ref.runGet? σ = .some e
...
lemma eval_pushExpr {ref : HashConsM p ExprRef}
  (h : ref.run' (σ.pushExpr (.c k) wellFormed_c) = σ.exprs.size)
:
  ([varStore|ref].run (σ.pushExpr (.c k) wellFormed_c)).run =
  (.some k, (ref.run σ).run.2)
:= by
  simp
...
lemma eval_c
  ...
  {σ : HashConsSt p}
:
  [varStore,σ|mkConstant k] = some k
:= by
  simp [eval]
