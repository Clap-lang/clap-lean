...
class Convert (p : ℕ) (representsT idealT : Type) extends IsValid p representsT where
  ...
  toIdeal_toRepresents :
    ∀ (varStore : VarStore p) (σ : HashConsSt p) (x : idealT),
      toIdeal varStore ((toRepresents x).run σ).2 ((toRepresents x).run σ).1 = .some x
...
:
  toIdeal varStore σ (toRepresents p (representsT := representsT) ((toIdeal (idealT := idealT) varStore σ x).get ((isValid_iff_isSome_toIdeal varStore x).mp h))) =
