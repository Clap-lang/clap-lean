...
@[grind _=_]
lemma wellFormed_iff_isSome : e.wellFormed ↔ (*ₑe).isSome := by grind [=wellFormed]
...
:
  *ₑ(Expr.mk σ.size (σ.pushExpr e h)) =
  .some e
:= by
  grind
...
:
  *ₑ⦃σ.exprs.idxOf cacheExpr, σ⦄ = cacheExpr
:= by
  grind
...
lemma deref_mkConstant_eq_some
  ...
  {σ : HashConsSt p}
:
  *ₑ⦃(HashConsM.mkConstant c).getResult σ, (HashConsM.mkConstant c).getHashConsState σ⦄ =
  .some (CacheExpr.c c)
:= by
  grind
...
lemma deref_mkVar_eq_some
  ...
  {σ : HashConsSt p}
:
  *ₑ⦃(HashConsM.mkVar idx).getResult σ, (HashConsM.mkVar idx).getHashConsState σ⦄ =
...
lemma deref_mkAdd_eq_some
  ...
  (h_r : ⦃r,σ⦄.wellFormed)
:
  *ₑ⦃(HashConsM.mkAdd l r).getResult σ, (HashConsM.mkAdd l r).getHashConsState σ⦄ =
...
lemma deref_mkSub_eq_some
  ...
  (h_r : ⦃r,σ⦄.wellFormed)
:
  *ₑ⦃(HashConsM.mkSub l r).getResult σ, (HashConsM.mkSub l r).getHashConsState σ⦄ =
...
lemma deref_mkMul_eq_some
  ...
  (h_r : ⦃r,σ⦄.wellFormed)
:
  *ₑ⦃(HashConsM.mkMul l r).getResult σ, (HashConsM.mkMul l r).getHashConsState σ⦄ =
