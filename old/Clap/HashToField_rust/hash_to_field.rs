...
use aptos_crypto::{
    poseidon_bn254::{
        ...
        hash_scalars, pad_and_hash_bytes_with_len, MAX_NUM_INPUT_SCALARS,
    },
...
/// Pure (no circom) — prints reference vectors for Clap/HashToField.lean.
/// Run: cargo test -p aptos-keyless-circuit print_hashtofield_vectors -- --nocapture
#[test]
fn print_hashtofield_vectors() {
    fn emit_bytes(label: &str, msg: &[u8], capacity: usize) {
        let out = pad_and_hash_bytes_with_len(msg, capacity).unwrap();
        println!(
            "BYTES {label}: msg={:?} len={} capacity={} output={}",
            msg,
            msg.len(),
            capacity,
            out
    ...
    fn emit_limbs(label: &str, limbs: &[u64], capacity: usize) {
        let out = pad_and_hash_limbs_with_len(limbs, capacity).unwrap();
        println!(
            "LIMBS {label}: limbs={:?} len={} capacity={} output={}",
            limbs,
            limbs.len(),
            capacity,
            out
    ...
    // raw hashElemsToField reference (single Poseidon for <= 16 elems)
    fn emit_elems(label: &str, elems: &[u64]) {
        let frs: Vec<Fr> = elems.iter().map(|&x| Fr::from(x)).collect();
        let out = hash_scalars(frs).unwrap();
        println!("ELEMS {label}: elems={:?} output={}", elems, out);
    }
    // bytes
    emit_bytes("f_replace", &[147, 139, 223, 159, 166, 20], 6); // single scalar, no padding
    emit_bytes("b_pad", &[1, 2, 3, 4, 5], 62); // 2 scalars (2nd = 0 padding)
    emit_bytes("c_two", (1u8..=40u8).collect::<Vec<u8>>().as_slice(), 62); // 2 scalars (full+partial)
                                                                           // limbs (keyless len = numLimbs)
    emit_limbs("l1", &[1, 0, 0, 2, 0, 0], 6); // 2 scalars -> [1, 2, 6]
    emit_limbs("l2", &[0, 1, 0], 3); // 1 scalar  -> [2^64, 3]
    emit_limbs("l3", &[1, 2], 6); // 2 scalars (2nd = 0) -> [1+2*2^64, 0, 2]
    // raw hashElemsToField inputs for the existing chunk31 / #v[0,0,0] tests
    // (their values went stale when hashElemsToField stopped double-hashing n<=16)
    emit_elems("e_000", &[0, 0, 0]); // hashElemsToField #v[0,0,0]
    emit_elems("e_0_1_62", &[0, 1, 62]); // hashBytesToField chunk31Zero ++ chunk31One, len 62
    emit_elems("e_0_0_62", &[0, 0, 62]); // hashBytesToField chunk31Zero ++ chunk31Zero, len 62
}
